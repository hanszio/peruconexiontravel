...
$table_name = $trustindex_pm_tripadvisor->get_tablename('reviews');
...
if($request_id)
{
update_option($trustindex_pm_tripadvisor->get_option_name('review-download-request-id'), $request_id, false);
}
$manual_download = 0;
...
if($review_download)
{
update_option($trustindex_pm_tripadvisor->get_option_name('review-download-inprogress'), $review_download, false);
update_option($trustindex_pm_tripadvisor->get_option_name('review-manual-download'), $manual_download, false);
}
else
{
...
if($trustindex_pm_tripadvisor->shortname == 'facebook' || count($reviews) == $page_details['rating_number'] || count($reviews) == 10)
{
...
{
check_admin_referer( 'save-align_'.$trustindex_pm_tripadvisor->get_plugin_slug(), '_wpnonce_align' );
...
{
check_admin_referer( 'save-review-text-mode_'.$trustindex_pm_tripadvisor->get_plugin_slug(), '_wpnonce_review_text_mode' );
...
update_option($trustindex_pm_tripadvisor->get_option_name('rate-us-feedback'), $star, false);
if($star > 3)
{
header('Location: https://wordpress.org/support/plugin/'. $trustindex_pm_tripadvisor->get_plugin_slug() . '/reviews/?rate='. $star .'#new-post');
}
else
{
wp_mail('support@trustindex.io', 'Feedback from Tripadvisor plugin', "We received a <strong>$star star</strong> feedback about the Tripadvisor plugin from $email:<br /><br />$text", [
'From: '. $email,
...
$no_rating_text = get_option( $trustindex_pm_tripadvisor->get_option_name('no-rating-text'), $trustindex_pm_tripadvisor->get_default_no_rating_text($style_id, $scss_set) );
...
$align = get_option( $trustindex_pm_tripadvisor->get_option_name('align'), in_array($style_id, [ 36, 37, 38, 39 ]) ? 'center' : 'left' );
$review_text_mode = get_option( $trustindex_pm_tripadvisor->get_option_name('review-text-mode'), 'readmore' );
$scss_set_tmp = $scss_set ? $scss_set : 'light-background';
$show_reviewers_photo = get_option( $trustindex_pm_tripadvisor->get_option_name('show-reviewers-photo'), TrustindexPlugin_tripadvisor::$widget_styles[$scss_set_tmp]['reviewer-photo'] ? 1 : 0 );
$show_logos = get_option( $trustindex_pm_tripadvisor->get_option_name('show-logos'), TrustindexPlugin_tripadvisor::$widget_styles[$scss_set_tmp]['hide-logos'] ? 0 : 1 );
$show_stars = get_option( $trustindex_pm_tripadvisor->get_option_name('show-stars'), TrustindexPlugin_tripadvisor::$widget_styles[$scss_set_tmp]['hide-stars'] ? 0 : 1 );
$rate_us_feedback = get_option( $trustindex_pm_tripadvisor->get_option_name('rate-us-feedback'), 0 );
