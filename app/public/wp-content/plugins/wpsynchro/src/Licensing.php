...
class Licensing
{
    ...
    public $migration_server_service = "https://wpsynchro.com/api/v1/synchronizerequest/";
    ...
    public function verifyLicenseForMigration($from_url, $to_url)
    {
        ...
        if ($verified_license) {
            ...
            $remotetransport = $wpsynchro_container->get('class.RemoteTransfer');
            $remotetransport->init();
            $remotetransport->setUrl($this->migration_server_service . $this->getCurrentLicenseKey());
            $remotetransport->setDataObject($bodydata);
            $remotetransport->setSendDataAsJSON();
            ...
            if ($sync_request_result->isSuccess()) {
                ...
                if (isset($body->valid) && $body->valid === true) {
                    ...
                    $result->state = false;
                    $result->errors[] = __("Migration was denied by WP Synchro license server, which can be caused by invalid license key or not having enough available sites on your current subscription. Log into https://wpsynchro.com to check out your subscription state.", "wpsynchro");
                }
            } else {
                $result->state = false;
