<?php
if ( !class_exists('WPSynchroPuc_v4p9_Plugin_Ui', false) ):
	/**
	 * Additional UI elements for plugins.
	 */
	class WPSynchroPuc_v4p9_Plugin_Ui {
		private $updateChecker;
		private $manualCheckErrorTransient = '';

		/**
		 * @param WPSynchroPuc_v4p9_Plugin_UpdateChecker $updateChecker
		 */
		 ...
		 *
		 * You can change the link text by using the "WPSynchroPuc_view_details_link-$slug" filter.
		 * Returning an empty string from the filter will disable the link.
		 *
		 * You can change the position of the link using the
		 * "WPSynchroPuc_view_details_link_position-$slug" filter.
		 ...
		 *
		 * You can change the link text by using the "WPSynchroPuc_manual_check_link-$slug" filter.
		 * Returning an empty string from the filter will disable the link.
		 *
		 * @param array $pluginMeta Array of meta links.
		 * @param string $pluginFile
		...
		public function addCheckForUpdatesLink($pluginMeta, $pluginFile) {
			if ( $this->isMyPluginFile($pluginFile) ) {
				$linkUrl = wp_nonce_url(
					add_query_arg(
						array(
							'WPSynchroPuc_check_for_updates' => 1,
							'WPSynchroPuc_slug'              => $this->updateChecker->slug,
						),
						self_admin_url('plugins.php')
					),
					'WPSynchroPuc_check_for_updates'
				);
		...
		public function handleManualCheck() {
			$shouldCheck =
				isset($_GET['WPSynchroPuc_check_for_updates'], $_GET['WPSynchroPuc_slug'])
				&& $_GET['WPSynchroPuc_slug'] == $this->updateChecker->slug
				&& check_admin_referer('WPSynchroPuc_check_for_updates');
			...
			if ( $shouldCheck ) {
				...
				wp_redirect(add_query_arg(
					array(
						'WPSynchroPuc_update_check_result' => $status,
						'WPSynchroPuc_slug'                => $this->updateChecker->slug,
					),
					self_admin_url('plugins.php')
				));
		 ...
		 *
		 * You can change the result message by using the "WPSynchroPuc_manual_check_message-$slug" filter.
		 */
		public function displayManualCheckResult() {
			if ( isset($_GET['WPSynchroPuc_update_check_result'], $_GET['WPSynchroPuc_slug']) && ($_GET['WPSynchroPuc_slug'] == $this->updateChecker->slug) ) {
				$status = strval($_GET['WPSynchroPuc_update_check_result']);
