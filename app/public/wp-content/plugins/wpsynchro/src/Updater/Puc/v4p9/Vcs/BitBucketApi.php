<?php
if ( !class_exists('WPSynchroPuc_v4p9_Vcs_BitBucketApi', false) ):

	class WPSynchroPuc_v4p9_Vcs_BitBucketApi extends WPSynchroPuc_v4p9_Vcs_Api {
		/**
		 * @var WPSynchroPuc_v4p9_OAuthSignature
		 */
		private $oauth = null;
		 ...
		 * @return null|WPSynchroPuc_v4p9_Vcs_Reference
		 */
		...
		public function getBranch($branchName) {
			...
			return new WPSynchroPuc_v4p9_Vcs_Reference(array(
				'name' => $branch->name,
		 ...
		 * @return WPSynchroPuc_v4p9_Vcs_Reference|null
		 */
		...
		public function api($url, $version = '2.0') {
			$url = ltrim($url, '/');
			$isSrcResource = WPSynchroPuc_v4p9_Utils::startsWith($url, 'src/');
			...
			if ( is_wp_error($response) ) {
				do_action('WPSynchroPuc_api_error', $response, null, $url, $this->slug);
				return $response;
			}
			...
			do_action('WPSynchroPuc_api_error', $error, $response, $url, $this->slug);
		...
		public function setAuthentication($credentials) {
			...
			if ( !empty($credentials) && !empty($credentials['consumer_key']) ) {
				$this->oauth = new WPSynchroPuc_v4p9_OAuthSignature(
