...
if ( !class_exists('WPSynchroPuc_v4p9_StateStore', false) ):

	class WPSynchroPuc_v4p9_StateStore {
		/**
		...
		/**
		 * @var WPSynchroPuc_v4p9_Update|null Cached update.
		 */
		...
		/**
		 * @return null|WPSynchroPuc_v4p9_Update
		 */
		 ...
		 * @return $this
		 */
		public function setUpdate(WPSynchroPuc_v4p9_Update $update = null) {
			$this->lazyLoad();
			$this->update = $update;
			return $this;
		}
		...
		public function save() {
			...
			if ( isset($this->update)) {
				...
				if ( WPSynchroPuc_v4p9_Utils::startsWith($updateClass, $prefix) ) {
		...
		protected function load() {
			...
			$this->lastCheck = intval(WPSynchroPuc_v4p9_Utils::get($state, 'lastCheck', 0));
			$this->checkedVersion = WPSynchroPuc_v4p9_Utils::get($state, 'checkedVersion', '');
			$this->update = null;
