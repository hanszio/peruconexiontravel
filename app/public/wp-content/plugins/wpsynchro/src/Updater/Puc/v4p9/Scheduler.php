<?php
if ( !class_exists('WPSynchroPuc_v4p9_Scheduler', false) ):

	/**
	 * The scheduler decides when and how often to check for updates.
	 * It calls @see WPSynchroPuc_v4p9_UpdateChecker::checkForUpdates() to perform the actual checks.
	 */
	class WPSynchroPuc_v4p9_Scheduler {
		 ...
		 *
		 * @param WPSynchroPuc_v4p9_UpdateChecker $updateChecker
		...
		public function upgraderProcessComplete(
			...
			if ( is_a($this->updateChecker, 'WPSynchroPuc_v4p9_Theme_UpdateChecker') ) {
			...
			if ( is_a($this->updateChecker, 'WPSynchroPuc_v4p9_Plugin_UpdateChecker') ) {
		 ...
		 *
		 * You can override the default behaviour by using the "WPSynchroPuc_check_now-$slug" filter.
