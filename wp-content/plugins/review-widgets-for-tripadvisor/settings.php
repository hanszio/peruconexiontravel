...
case 'later':
$trustindex_pm_tripadvisor->setNotificationParam($type, 'timestamp', time() + (14 * 86400));
break;
case 'close':
if ($type !== 'rate-us') {
$trustindex_pm_tripadvisor->setNotificationParam($type, 'active', false);
}
break;
case 'open':
$trustindex_pm_tripadvisor->setNotificationParam($type, 'active', false);
if ($type === 'rate-us') {
header('Location: https://wordpress.org/support/plugin/'. $trustindex_pm_tripadvisor->get_plugin_slug() . '/reviews/?rate=5#new-post');
exit;
}
$tab = 'setup_no_reg';
...
break;
case 'hide':
$trustindex_pm_tripadvisor->setNotificationParam($type, 'hidden', true);
header('Location: admin.php?page=' . sanitize_text_field($_GET['page']) .'&tab=advanced');
break;
case 'unhide':
$trustindex_pm_tripadvisor->setNotificationParam($type, 'hidden', false);
header('Location: admin.php?page=' . sanitize_text_field($_GET['page']) .'&tab=advanced');
break;
}
exit;
...
$tabs[ TrustindexPlugin_tripadvisor::___('Free Widget Configurator') ] = 'setup_no_reg';
}
else {
...
$tabs[ TrustindexPlugin_tripadvisor::___('My reviews') ] = 'my_reviews';
}
$tabs[ TrustindexPlugin_tripadvisor::___('Get Reviews') ] = 'get_reviews';
$tabs[ TrustindexPlugin_tripadvisor::___('Rate Us') ] = 'rate';
if (!$trustindex_pm_tripadvisor->is_trustindex_connected()) {
$tabs[ TrustindexPlugin_tripadvisor::___('Get more Features') ] = 'setup_trustindex';
$tabs[ TrustindexPlugin_tripadvisor::___('Log In') ] = 'setup_trustindex_join';
}
$tabs[ TrustindexPlugin_tripadvisor::___('Advanced') ] = 'advanced';
$tabs[ TrustindexPlugin_tripadvisor::___('Feature request') ] = 'feature_request';
$selectedTab = isset($_GET['tab']) ? sanitize_text_field($_GET['tab']) : null;
$subTabs = null;
$newBadgeTabs = [];
if (get_option($trustindex_pm_tripadvisor->get_option_name('widget-setted-up'), 0) && !get_option($trustindex_pm_tripadvisor->get_option_name('reply-generated'), 0)) {
...
$proxyCheck = true;
if ($proxy->is_enabled()) {
$optName = $trustindex_pm_tripadvisor->get_option_name('proxy-check');
...
<p>
<?php echo TrustindexPlugin_tripadvisor::___('For some reason, the <strong>CSS</strong> file required to run the plugin was not loaded.<br />One of your plugins is probably causing the problem.'); ?>
</p>
</div>
...
let jsFiles = [
{
url: '<?php echo $trustindex_pm_tripadvisor->get_plugin_file_url('static/js/admin-page-settings-connect.js'); ?>',
id: 'connect'
},
{
url: '<?php echo $trustindex_pm_tripadvisor->get_plugin_file_url('static/js/admin-page-settings-common.js'); ?>',
id: 'common'
},
<?php if(in_array($trustindex_pm_tripadvisor->shortname, [ 'google', 'facebook' ])): ?>
{
url: '<?php echo $trustindex_pm_tripadvisor->get_plugin_file_url('static/js/admin-page-settings.js'); ?>',
id: 'unique'
}
<?php endif; ?>
];
...
element.href = url;
element.id = 'trustindex_settings_style_tripadvisor-css';
}
...
let link = document.getElementById('trustindex_settings_style_tripadvisor-css');
...
if (!isCSSExists()) {
addElement('link', '<?php echo $trustindex_pm_tripadvisor->get_plugin_file_url('static/css/admin-page-settings.css'); ?>', function(success) {
loadedCount++;
if (!success) {
notLoaded.push('CSS');
}
...
<?php echo TrustindexPlugin_tripadvisor::___("WP Tripadvisor Review Widgets"); ?>
...
<p>
<?php echo TrustindexPlugin_tripadvisor::___('Your site cannot download our widget templates, because of your server settings not allowing that:'); ?><br /><a href="https://wordpress.org/support/article/editing-wp-config-php/#block-external-url-requests" target="_blank">https://wordpress.org/support/article/editing-wp-config-php/#block-external-url-requests</a><br /><br />
<strong><?php echo TrustindexPlugin_tripadvisor::___('Solution'); ?></strong><br />
<?php echo TrustindexPlugin_tripadvisor::___('a) You should define <strong>WP_HTTP_BLOCK_EXTERNAL</strong> as false'); ?><br />
<?php echo TrustindexPlugin_tripadvisor::___("b) or you should add Trustindex as an <strong>WP_ACCESSIBLE_HOSTS</strong>: \"*.trustindex.io\""); ?><br />
</p>
</div>
<?php endif; ?>
...
<div class="ti-box ti-notice-error">
<p>
<?php echo TrustindexPlugin_tripadvisor::___('It seems you are using a proxy for HTTP requests but after a test request it returned a following error:'); ?><br />
<strong><?php echo $proxyCheck; ?></strong><br /><br />
<?php echo TrustindexPlugin_tripadvisor::___('Therefore, our plugin might not work properly. Please, contact your hosting support, they can resolve this easily.'); ?>
</p>
<a href="<?php echo wp_nonce_url('?page='. esc_attr($_GET['page']) .'&tab='. esc_attr($_GET['tab']) .'&test_proxy', 'ti-test-proxy'); ?>" class="btn-text btn-refresh"><?php echo TrustindexPlugin_tripadvisor::___('Test again') ;?></a>
</div>
<?php endif; ?>
...
<span class="ti-new-badge"><?php echo TrustindexPlugin_tripadvisor::___('new'); ?></span>
<?php endif; ?>
</a>
<?php endforeach; ?>
</div>
