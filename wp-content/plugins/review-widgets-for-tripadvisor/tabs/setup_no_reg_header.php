...
$tableName = $trustindex_pm_tripadvisor->get_tablename('reviews');
...
if ($requestId) {
update_option($trustindex_pm_tripadvisor->get_option_name('review-download-request-id'), $requestId, false);
}
$manualDownload = 0;
...
if ($reviewDownload) {
update_option($trustindex_pm_tripadvisor->get_option_name('review-download-inprogress'), $reviewDownload, false);
update_option($trustindex_pm_tripadvisor->get_option_name('review-manual-download'), $manualDownload, false);
update_option($trustindex_pm_tripadvisor->get_option_name('review-download-is-connecting'), 1, false);
}
else {
...
if ($trustindex_pm_tripadvisor->shortname == 'facebook' || count($reviews) == $pageDetails['rating_number'] || count($reviews) === 10) {
$timestamp = time() + (86400 * 10);
if (isset($pageDetails['timestamp'])) {
$timestamp = $pageDetails['timestamp'];
unset($pageDetails['timestamp']);
}
update_option($trustindex_pm_tripadvisor->get_option_name('download-timestamp'), $timestamp, false);
delete_option($trustindex_pm_tripadvisor->get_option_name('review-download-inprogress'));
delete_option($trustindex_pm_tripadvisor->get_option_name('review-manual-download'));
update_option($trustindex_pm_tripadvisor->get_option_name('review-download-modal'), 0, false);
}
}
update_option($trustindex_pm_tripadvisor->get_option_name('page-details'), $pageDetails, false);
$GLOBALS['wp_object_cache']->delete( $trustindex_pm_tripadvisor->get_option_name('page-details'), 'options' );
if ($defaultSettings) {
$lang = strtolower(substr(get_locale(), 0, 2));
if (!isset(TrustindexPlugin_tripadvisor::$widget_languages[ $lang ])) {
$lang = 'en';
}
...
global $trustindex_pm_tripadvisor;
global $wpdb;
$trustindex_pm_tripadvisor->delete_async_request();
delete_option($trustindex_pm_tripadvisor->get_option_name('review-download-inprogress'));
delete_option($trustindex_pm_tripadvisor->get_option_name('review-download-request-id'));
delete_option($trustindex_pm_tripadvisor->get_option_name('review-manual-download'));
...
unlink($trustindex_pm_tripadvisor->getCssFile());
}
if ($settingsDelete) {
...
delete_option($trustindex_pm_tripadvisor->get_option_name('show-header-button'));
delete_option($trustindex_pm_tripadvisor->get_option_name('reviews-load-more'));
...
$wpdb->query('TRUNCATE `'. $trustindex_pm_tripadvisor->get_tablename('reviews') .'`');
$trustindex_pm_tripadvisor->setNotificationParam('not-using-no-connection', 'active', true);
$trustindex_pm_tripadvisor->setNotificationParam('not-using-no-connection', 'timestamp', time() + 86400);
$trustindex_pm_tripadvisor->setNotificationParam('not-using-no-widget', 'active', false);
}
...
delete_option($trustindex_pm_tripadvisor->get_option_name($name));
}
}
if ($step < 4) {
delete_option($trustindex_pm_tripadvisor->get_option_name('scss-set'));
...
update_option($trustindex_pm_tripadvisor->get_option_name('style-id'), $styleId, false);
delete_option($trustindex_pm_tripadvisor->get_option_name('review-content'));
...
update_option($trustindex_pm_tripadvisor->get_option_name('show-header-button'), $r, false);
$r = 1;
if (isset($_POST['reviews-load-more'])) {
$r = sanitize_text_field($_POST['reviews-load-more']);
}
update_option($trustindex_pm_tripadvisor->get_option_name('reviews-load-more'), $r, false);
$r = 1;
...
update_option($trustindex_pm_tripadvisor->get_option_name('footer-filter-text'), $r, false);
delete_option($trustindex_pm_tripadvisor->get_option_name('review-content'));
$trustindex_pm_tripadvisor->noreg_save_css(true);
exit;
}
...
update_option($trustindex_pm_tripadvisor->get_option_name('align'), sanitize_text_field($_POST['align']), false);
$trustindex_pm_tripadvisor->noreg_save_css(true);
exit;
}
else if ($ti_command === 'save-review-text-mode') {
check_admin_referer('ti-save-review-text-mode');
update_option($trustindex_pm_tripadvisor->get_option_name('review-text-mode'), sanitize_text_field($_POST['review_text_mode']), false);
$trustindex_pm_tripadvisor->noreg_save_css(true);
exit;
}
else if ($ti_command === 'save-amp-notice-hide') {
update_option($trustindex_pm_tripadvisor->get_option_name('amp-hidden-notification'), 1, false);
exit;
}
...
'body' => [ 'id' => get_option($trustindex_pm_tripadvisor->get_option_name('review-download-request-id')) ],
'timeout' => '30',
'redirection' => '5',
'blocking' => true
]);
...
$trustindex_pm_tripadvisor->save_details($json['details']);
$trustindex_pm_tripadvisor->save_reviews(isset($json['reviews']) ? $json['reviews'] : []);
delete_option($trustindex_pm_tripadvisor->get_option_name('review-download-token'));
delete_option($trustindex_pm_tripadvisor->get_option_name('review-download-inprogress'));
delete_option($trustindex_pm_tripadvisor->get_option_name('review-manual-download'));
...
$trustindex_pm_tripadvisor->activate();
header('Location: admin.php?page=' . sanitize_text_field($_GET['page']) .'&tab=setup_no_reg');
exit;
}
...
$styleId = get_option($trustindex_pm_tripadvisor->get_option_name('style-id'));
$scssSet = get_option($trustindex_pm_tripadvisor->get_option_name('scss-set'));
...
$noRatingText = get_option($trustindex_pm_tripadvisor->get_option_name('no-rating-text'), $trustindex_pm_tripadvisor->get_default_no_rating_text($styleId, $scssSet));
$filter = get_option($trustindex_pm_tripadvisor->get_option_name('filter'), $trustindex_pm_tripadvisor->get_widget_default_filter());
$verifiedIcon = get_option($trustindex_pm_tripadvisor->get_option_name('verified-icon'), 0);
$enableAnimation = get_option($trustindex_pm_tripadvisor->get_option_name('enable-animation'), 1);
$showArrows = get_option($trustindex_pm_tripadvisor->get_option_name('show-arrows'), 1);
$showHeaderButton = get_option($trustindex_pm_tripadvisor->get_option_name('show-header-button'), 1);
$reviewsLoadMore = get_option($trustindex_pm_tripadvisor->get_option_name('reviews-load-more'), 1);
$widgetSettedUp = get_option($trustindex_pm_tripadvisor->get_option_name('widget-setted-up'), 0);
$disableFont = get_option($trustindex_pm_tripadvisor->get_option_name('disable-font'), 0);
$align = get_option($trustindex_pm_tripadvisor->get_option_name('align'), in_array($styleId, [ 36, 37, 38, 39 ]) ? 'center' : 'left');
$reviewTextMode = get_option($trustindex_pm_tripadvisor->get_option_name('review-text-mode'), 'readmore');
$footerFilterText = get_option($trustindex_pm_tripadvisor->get_option_name('footer-filter-text'), 0);
$scssSetTmp = $scssSet ? $scssSet : 'light-background';
$showReviewersPhoto = get_option($trustindex_pm_tripadvisor->get_option_name('show-reviewers-photo'), TrustindexPlugin_tripadvisor::$widget_styles[ $scssSetTmp ]['reviewer-photo'] ? 1 : 0);
$showLogos = get_option($trustindex_pm_tripadvisor->get_option_name('show-logos'), TrustindexPlugin_tripadvisor::$widget_styles[ $scssSetTmp ]['hide-logos'] ? 0 : 1);
$showStars = get_option($trustindex_pm_tripadvisor->get_option_name('show-stars'), TrustindexPlugin_tripadvisor::$widget_styles[ $scssSetTmp ]['hide-stars'] ? 0 : 1);
